...
public class VendorProductServiceImpl implements VendorProductService{
	...
	@Override
	public VendorProductDto getVendorProdById(Long vendProdId) {
		VendorProducts venProd = venProdRepo.findById(vendProdId)
				.orElseThrow(()-> new ResourceNotFoundException("Invalid prod id"));
		VendorProductDto dto = mapper.map(venProd, VendorProductDto.class);
		dto.setCategoryId(venProd.getCategory().getCategoryId());
		dto.setSubCatId(venProd.getSubcategory().getSubCatId());
		dto.setVendorId(venProd.getVendor().getVendorId());
